...
func (comm *Comm) StartClient(addr string, key, cert, ca []byte) error {
	// fmt.Printf("comm[%p].StartClient(addr: %v)", comm, addr)
	...
	comm.client = client
	fmt.Printf("StartClient success")
...
func (comm *Comm) StopClient() {
	fmt.Printf("comm[%p].StopClient()", comm)
	if comm.client != nil {
		comm.client.close()
	}
}
...
func (comm *Comm) Subscribe(topics string) {
	fmt.Printf("comm[%p]client.Subscribe(%v)", comm, topics)
	m := sjson.New()
	m.Set("cmd", "subscribe")
	m.Set("topics", topics)
	if comm.client != nil && comm.client.enc != nil {
		...
		fmt.Printf("comm is disconnected")
	}
}

// Unsubscribe comm客户端取消订阅主题
func (comm *Comm) Unsubscribe(topics string) {
	fmt.Printf("comm[%p]client.Unsubscribe(%v)", comm, topics)
	m := sjson.New()
...
// Publish comm客户端发布消息
func (comm *Comm) Publish(topic string, m *sjson.Json) {
	fmt.Printf("comm[%p]client.Publish(%v, %v)", comm, topic, m)
	m.Set("cmd", "publish")
	m.Set("topic", topic)
	if comm.client != nil && comm.client.enc != nil {
		comm.client.send(m)
...
func connect(comm *Comm, addr string, key, cert, ca []byte) (*commClient, error) {
	...
	if err != nil {
		fmt.Printf("Dial(%v) error: %v", addr, err)
		return nil, err
	}
	// setkeepalive
	...
	err = conn.Handshake()
	if err != nil {
		fmt.Printf("connect handshake error: %v", err)
		return nil, err
	}
	fmt.Printf("connect handshake success")
	client := &commClient{
		conn: conn,
		topics: &commTopics{
	...
	go func() {
		...
		for {
			...
			if err != nil {
				...
				if c.server == nil {
					fmt.Printf("comm[%p]client disconnected: %v", comm, err)
				} else {
					fmt.Printf("comm[%p]peer disconnected: %v", comm, err)
				}
				errStr := err.Error()
...
func (c *commClient) handleCmd(msg *sjson.Json) {
	fmt.Printf("comm[%p]client/peer.handleCmd(%v)", c.comm, msg)
...
func (c *commClient) send(v interface{}) error {
	fmt.Printf("comm[%p]client/peer.Send() %v", c.comm, v)
